...
export function addNextNotification(activeAlarm, location) {
    ...
    let nextZmanDate = AppUtils.getNextZmanTime(zmanType.name, now, location),
        nextAlarmDate = new Date(nextZmanDate.getTime() + (activeAlarm.alarmOffset * 60000));
    if (activeAlarm.days.length && !activeAlarm.days.includes(nextZmanDate.getDay())) {
        while (!activeAlarm.days.includes(nextZmanDate.getDay()) || now.getTime() >= nextAlarmDate.getTime()) {
            nextZmanDate = AppUtils.getNextZmanTime(activeAlarm.zmanName, Utils.addDaysToSdate(1), location);
            nextAlarmDate = new Date(nextZmanDate.getTime() + (activeAlarm.alarmOffset * 60000));
        }
    }
